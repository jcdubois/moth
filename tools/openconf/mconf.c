...
static const char
    mconf_readme[] = N_(
        ...
        "menuconfig\n"
        "with " OPENCONF_MENUCONFIG_MODE_ENVNAME
        " environment variable set to single_menu. Example:\n"
        "\n"
        "make " OPENCONF_MENUCONFIG_MODE_ENVNAME "=single_menu menuconfig\n"
        "\n"
        "<Enter> will then unroll the appropriate category, or enfold it if "
        "it\n"
        "is already unrolled.\n"
        "\n"
        ...
        "--------------------------------\n"
        "It is possible to select different color themes using the "
        "variable\n" OPENCONF_MENUCONFIG_COLOR_ENVNAME
        ". To select a theme use:\n"
        "\n"
    ...
    inputbox_instructions_int[] = N_("Please enter a decimal value. "
                                     ...
                                     "Use the <TAB> key to move from the input "
                                     "field to the buttons below it."),
    inputbox_instructions_hex[] = N_("Please enter a hexadecimal value. "
                                     "Use the <TAB> key to move from the input "
                                     "field to the buttons below it."),
    inputbox_instructions_string[] = N_("Please enter a string value. "
                                        "Use the <TAB> key to move from the "
                                        "input field to the buttons below it."),
    setmod_text[] = N_("This feature depends on another which has been "
                       "configured as a module.\n"
                       "As a result, this feature will be built as a module."),
    nohelp_text[] = N_("There is no help available for this option.\n"),
    load_config_text[] =
        N_("Enter the name of the configuration file you wish to load.  "
    ...
    load_config_help[] = N_(
        ...
        "abort.\n"),
    save_config_text[] =
        N_("Enter a filename to which this configuration should be saved "
...
static void get_prompt_str(struct gstr *r, struct property *prop) {
  ...
  if (i > 0) {
    ...
    for (j = 4; --i >= 0; j += 2) {
      ...
      if (menu->sym) {
        str_printf(r, " (%s [=%s])",
                   menu->sym->name ? menu->sym->name : _("<choice>"),
...
static void conf_save(void) {
  while (1) {
    ...
    case 0:
      ...
      show_textbox(NULL,
                   _("Can't create file!  Probably a nonexistent directory."),
                   5, 60);
      break;
    case 1:
