 ...
 */
bool ConfigSettings::writeSizes(const QString &key,
...
void ConfigItem::paintCell(QPainter *p, const QColorGroup &cg, int column,
                           int width, int align) {
...
ConfigList::ConfigList(ConfigView *p, const char *name)
    : Parent(p, name), updateAll(false), symbolYesPix(xpm_symbol_yes),
      symbolModPix(xpm_symbol_mod), symbolNoPix(xpm_symbol_no),
      choiceYesPix(xpm_choice_yes), choiceNoPix(xpm_choice_no),
      menuPix(xpm_menu), menuInvPix(xpm_menu_inv), menuBackPix(xpm_menuback),
      voidPix(xpm_void), showAll(false), showName(false), showRange(false),
      showData(false), rootEntry(0), headerPopup(0) {
  int i;
  ...
  connect(this, SIGNAL(selectionChanged(void)), SLOT(updateSelection(void)));
  ...
  case promptColIdx:
    ...
    if (pm) {
      ...
      if (x >= off && x < off + pm->width()) {
        ...
        if (ptype == P_MENU && rootEntry != menu && mode != fullMode &&
            mode != menuMode) {
          emit menuSelected(menu);
        } else {
          changeValue(item);
...
void ConfigList::contextMenuEvent(QContextMenuEvent *e) {
  if (e->y() <= header()->geometry().bottom()) {
    if (!headerPopup) {
      ...
      connect(parent(), SIGNAL(showNameChanged(bool)), action,
              SLOT(setOn(bool)));
      action->setOn(showName);
      action->addTo(headerPopup);
      action = new QAction(NULL, _("Show Range"), 0, this);
      action->setToggleAction(TRUE);
      connect(action, SIGNAL(toggled(bool)), parent(),
              SLOT(setShowRange(bool)));
      connect(parent(), SIGNAL(showRangeChanged(bool)), action,
              SLOT(setOn(bool)));
      action->setOn(showRange);
      action->addTo(headerPopup);
      action = new QAction(NULL, _("Show Data"), 0, this);
      action->setToggleAction(TRUE);
      connect(action, SIGNAL(toggled(bool)), parent(), SLOT(setShowData(bool)));
      connect(parent(), SIGNAL(showDataChanged(bool)), action,
              SLOT(setOn(bool)));
...
void ConfigInfoView::menuInfo(void) {
  ...
  if (showDebug()) {
    debug += QString().sprintf("defined at %s:%d<br><br>", menu->file->name,
                               menu->lineno);
  }
...
ConfigSearchWindow::ConfigSearchWindow(ConfigMainWindow *parent,
  ...
  connect(list->list, SIGNAL(menuChanged(struct menu *)), info,
          SLOT(setInfo(struct menu *)));
  connect(list->list, SIGNAL(menuChanged(struct menu *)), parent,
  ...
  if (name) {
    ...
    height =
        configSettings->readNumEntry("/window height", parent->height() / 2);
    resize(width, height);
...
void ConfigSearchWindow::search(void) {
  ...
  for (p = result; *p; p++) {
    for_all_prompts((*p), prop) lastItem = new ConfigItem(
        list->list, lastItem, prop->menu, menu_is_visible(prop->menu));
  }
}
...
ConfigMainWindow::ConfigMainWindow(void) : searchWindow(0) {
  ...
  snprintf(title, sizeof(title), _("%s v%s Configuration"), name, version);
  setCaption(title);
  ...
  QAction *loadAction =
      new QAction("Load", QPixmap(xpm_load), _("&Load"), CTRL + Key_L, this);
  connect(loadAction, SIGNAL(activated()), SLOT(loadConfig()));
  saveAction =
      new QAction("Save", QPixmap(xpm_save), _("&Save"), CTRL + Key_S, this);
  ...
  QAction *singleViewAction = new QAction(
      "Single View", QPixmap(xpm_single_view), _("Single View"), 0, this);
  connect(singleViewAction, SIGNAL(activated()), SLOT(showSingleView()));
  QAction *splitViewAction = new QAction("Split View", QPixmap(xpm_split_view),
                                         _("Split View"), 0, this);
  connect(splitViewAction, SIGNAL(activated()), SLOT(showSplitView()));
  QAction *fullViewAction =
      new QAction("Full View", QPixmap(xpm_tree_view), _("Full View"), 0, this);
  ...
  connect(showAllAction, SIGNAL(toggled(bool)), configView,
          SLOT(setShowAll(bool)));
  connect(showAllAction, SIGNAL(toggled(bool)), menuView,
          SLOT(setShowAll(bool)));
  ...
  connect(configList, SIGNAL(menuChanged(struct menu *)), helpText,
          SLOT(setInfo(struct menu *)));
  connect(configList, SIGNAL(menuSelected(struct menu *)),
          SLOT(changeMenu(struct menu *)));
  connect(configList, SIGNAL(parentSelected()), SLOT(goBack()));
  connect(menuList, SIGNAL(menuChanged(struct menu *)), helpText,
          SLOT(setInfo(struct menu *)));
  connect(menuList, SIGNAL(menuSelected(struct menu *)),
          SLOT(changeMenu(struct menu *)));

  connect(configList, SIGNAL(gotFocus(struct menu *)), helpText,
          SLOT(setInfo(struct menu *)));
  connect(menuList, SIGNAL(gotFocus(struct menu *)), helpText,
          SLOT(setInfo(struct menu *)));
...
void ConfigMainWindow::showIntro(void) {
  static const QString str = _(
      ...
      "should be present, try turning on Show All Options under the Options "
      "menu.\n"
      "Although there is no cross reference yet to help you figure out what "
      "other\n"
      "options must be enabled to support the option you are interested in, "
      "you can\n"
...
void ConfigMainWindow::showAbout(void) {
  static const QString str =
      _("qconf is Copyright (C) 2002 Roman Zippel <zippel@linux-m68k.org>.\n\n"
        "Bug reports and feature request can also be entered at <some web "
        "address>\n");
