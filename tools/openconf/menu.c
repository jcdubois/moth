...
static struct menu **last_entry_ptr = NULL;
    ...
    case P_SELECT:
      ...
      if (sym->type != S_BOOLEAN && sym->type != S_TRISTATE) {
        prop_warn(prop,
                  ...
                  "not boolean or tristate",
                  sym->name);
      } else if (sym2->type != S_UNKNOWN && sym2->type != S_BOOLEAN &&
...
void menu_finalize(struct menu *parent) {
    ...
    if (sym && sym_is_choice(sym) && menu->sym &&
      ...
      if (sym->type == S_TRISTATE && menu->sym->type != S_TRISTATE) {
        ...
        for (prop = menu->sym->prop; prop; prop = prop->next) {
          ...
          prop->visible.expr =
              expr_alloc_and(expr_copy(basedep), prop->visible.expr);
        }
      }
  ...
  if (sym && !sym_is_optional(sym) && parent->prompt) {
    sym->rev_dep.expr = expr_alloc_or(
        sym->rev_dep.expr, expr_alloc_and(parent->prompt->visible.expr,
