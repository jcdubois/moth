...
static int conf_lineno = 0;
static int conf_warnings = 0;
static int conf_unsaved = 0;
...
int conf_read(const char *name) {
  ...
  sym_ok:
    ...
    expr_list_for_each_sym(prop->expr, e,
                           choice_sym) if (choice_sym->visible != no) flags &=
        choice_sym->flags;
...
int conf_write(const char *name) {
  ...
  struct menu *menu;
  const char *basename;
  char dirname[128], tmpname[256], newname[256], *project, *version;
  int type, l;
  const char *str;
  time_t now;
  int use_timestamp = 1;
