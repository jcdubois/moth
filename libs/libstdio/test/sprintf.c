...
#define TPRINTF(expr...)                                                       \
  ({                                                                           \
    ...
    s1 = malloc(1024);                                                         \
    s2 = malloc(1024);                                                         \
    sz1 = snprintf(s1, 1024, expr);                                            \
    sz2 = tfp_snprintf(s2, 256, expr);                                         \
    printf("libc_snprintf(%s) -> %d: ", #expr, (int)sz1);                      \
    puts(s1);                                                                  \
    printf(" tfp_snprintf(%s) -> %d: ", #expr, (int)sz2);                      \
    puts(s2);                                                                  \
    assert(sz1 == sz2);                                                        \
...
int main() {
  ...
  TPRINTF("42=%3d d1=%4.4x |%-4s| d2=%8.8x", 42, 0xd1, "string4", 0xd2);
  TPRINTF("84=%d 21=%ds |%s| |%sOK| d1=%x d2=%#x", 84, 21, "hello", "fine",
          0xd1, 0xd2);
