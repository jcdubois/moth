...
with Moth.Config; use Moth.Config;
with Moth.Current_task; use Moth.Current_task;
with Moth.Scheduler; use Moth.Scheduler;
...
is
   ...
   is
   begin
      os_task_list_mbx_fifo (task_id).mbx_array (get_mbx_head (task_id) + index) := mbx_entry;
   end set_mbx_entry;
   ...
   is (os_task_list_mbx_fifo (task_id).mbx_array (get_mbx_head (task_id) + index));
   ...
   function os_ghost_task_mbx_are_well_formed (task_id : os_task_id_param_t) return Boolean is
      (for all index in os_mbx_index_t'Range =>
         (if (os_mbx_count_t(index) >= get_mbx_count (task_id))
          then (os_task_list_mbx_fifo (task_id).mbx_array (get_mbx_head (task_id) + index).sender_id = OS_TASK_ID_NONE)
          else (os_task_list_mbx_fifo (task_id).mbx_array (get_mbx_head (task_id) + index).sender_id in os_task_id_param_t)))
   with
      Ghost => true,
